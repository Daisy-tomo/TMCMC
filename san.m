...
% 敏感参数先验上下界
lb = [0.84, 0.86, 0.98, 0.97];
ub = [0.86, 0.92, 0.995, 0.99];
n_params = length(lb);
...
% 敏感参数真值
theta_true = [0.86, 0.88, 0.988, 0.98];
...
function plot_results(results, theta_true, lb, ub, param_names)
    ...
    % 保存图片
    try
        saveas(fig1, '链轨迹图_4参数.png');
        saveas(fig2, '边缘后验图_4参数.png');
        fprintf('图片已保存：链轨迹图_4参数.png, 边缘后验图_4参数.png\n');
    catch
        fprintf('图形保存失败\n');
    end
end